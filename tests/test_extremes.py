...
def test_running_low_does_not_increase_within_day():
    coord = _make_coordinator()
    coord._merge_extremes(_stats(todays_low=55, todays_high=80), _NOW, None, None, current=None)
    out = coord._merge_extremes(
        _stats(todays_low=70, todays_high=82), _NOW, None, None, current=None
    )
...
def test_cache_resets_on_new_date():
    coord = _make_coordinator()
    coord._merge_extremes(_stats(todays_high=99, todays_low=50), _NOW, None, None, current=None)
    next_day = _NOW + timedelta(days=1)
    out = coord._merge_extremes(
        _stats(todays_high=70, todays_low=60), next_day, None, None, current=None
    )
...
def test_current_temperature_folds_into_day_bucket():
    coord = _make_coordinator()
    out = coord._merge_extremes(
        _stats(todays_high=80, todays_low=70), _NOW, None, None, current=82.5
    )
    assert out.todays_high == 82.5
    assert out.todays_low == 70
...
def test_current_temperature_classified_by_fixed_hours_when_no_sun():
    ...
    out = coord._merge_extremes(
        _stats(nighttime_high=70, nighttime_low=60),
        night_time,
        None,
        None,
        current=58,
    )
    assert out.nighttime_low == 58
...
def test_reset_extremes_drops_running_values():
    ...
    out = coord._merge_extremes(
        _stats(todays_high=70, todays_low=60), _NOW, None, None, current=None
    )
    assert out.todays_high == 70
    assert out.todays_low == 60
